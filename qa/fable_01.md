...
**F17 — the skill is a solo prompt again.** The `## /orchestrate` section documented extension internals (`drivePrAwait`, `armObservedLatch`), repeated one paragraph verbatim, and restated the writer contract in a file no writer is handed any more — Phase 2 removed `skills:` from `fixer.md` and `reviewFixLaunchParams`. It is replaced by a short `## Scope` block that says which sessions the skill is for and, for one that is not solo, that code owns the lifecycle and `git-workflow-guard` blocks the rest. The surviving writer contract is asserted against `parentGitWorkflowAppend`, the string code actually inlines.
...
**Still open after Phase 5.** The four Features parked at `phase: pr` with `pr: none` are unchanged — bookkeeping, not code, and there is no PR for the reconciler to ask about. This repo still has no `origin`, so the PR lifecycle cannot be exercised end to end here; since Phase 4 that at least refuses loudly at approve. §5's explicit state machine is not assigned to a phase; `FEATURE_PHASES` is the first half of it — the enum exists now, and what is missing is the single `transition(paths, from, to, reason)` writer, the illegal-move refusal, and `handoffs/transitions.log`.

### Cross-repo residuals — **CLOSED 2026-09-02**

Every `gh-pr-reviewer` item this review carried from Phase 1 onward is now closed, and two of the four were never open. **The list was wrong because it was read off a stale checkout**: `~/Dev/git/gh-pr-reviewer` sat six commits behind `origin/main` for the whole review, so four phases of "still open" were reported against code that had already changed. The lesson is cheap and worth stating: a residual in another repository has to be re-read against its remote before it is carried forward, not re-copied from the previous phase's list.

| Residual | Status |
|---|---|
| Rust waiter takes no per-PR lock (F3) | **Already landed.** `spawn_self_daemon` holds an exclusive kernel lock (`with_pid_lock`) across the live-pid check, the spawn, and the pidfile write, so two concurrent `--daemon` callers cannot both see a missing driver and race. A test pins the lock as preceding the spawn. |
| Naming change unlanded in `main` (F2) | **Already landed** in #276. `repo_file_key` / `latch_path_for` / `pid_path_for` / `log_path_for` exist; source and installed binary agree. TS keeps reading both spellings, which is still correct — 19 files on disk carry the qualified names and older ones the bare ones. |
| `ghl-wt` no-origin base + `--base` (4.3) | **Landed**, [#277](https://github.com/moofone/gh-pr-reviewer/pull/277). |
| `ghl-monitor.sh` respawns from `*.latch.json` (F3, F20) | **Landed**, [#278](https://github.com/moofone/gh-pr-reviewer/pull/278). |

**#277 — `ghl-wt`.** `parse_args` adds `--base REF` / `--base=REF` beside the legacy positional; `choose_base` picks explicit, else `origin/main` with a remote, else the repo's own default branch; `has_origin` gates the ls-remote probe and both fetches. Three review rounds of P2 findings from `codex-local-review`, each of them real and each the same principle the file already stated for `branch_exists` (fix-relay #3): don't collapse a failed probe into an answer. `classify_origin_probe` now distinguishes `git config --get` exit 1 ("key not set", the only outcome that means no remote) from exit 3/128 and a missing binary; `local_default_branch` propagates `branch_exists` errors instead of `unwrap_or(false)`; `--base` refuses a flag-shaped or empty value at the usage boundary rather than at `worktree_add_new`. 24 unit tests, each written red first.

**#278 — the monitor.** This one had been mis-scoped as an in-place edit to an untracked file, for the same stale-checkout reason: `scripts/ghl-monitor.sh` had been vendored into the repo in one of the six commits, so the local edit was silently reverted by the post-merge `setup-aliases.sh` refresh. Done properly it is one `case` skipping `*.latch.json`, with a test that puts the extension latch and the waiter's `manual-<pr>.json` for the same PR in the same directory and asserts the manual one still respawns — the skip is about the name, not the PR.

**Verified in production.** The monitor pass at 14:11Z logged `RESPAWNED driver for #2235 from pi-01a05f76-…latch.json` — the F20 defect firing, live, in the same minute the fix deployed. The 14:16Z pass no longer does. Deployment on this machine is by the post-merge hook, which runs `scripts/setup-aliases.sh`: `cargo build --release -p ghl-cli` and an atomic rename into `~/.local/bin`. `ghl-wt` there now reports `usage: ghl-wt <branch> [base-ref] [--base REF] [--yes]`. The review bots themselves needed no redeploy — they run the webhook units on the reviewer host, and neither changed file is part of them.
